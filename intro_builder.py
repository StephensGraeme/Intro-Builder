...
# ──────────────────────────────────────────────
APP_VERSION = "1.0"
NICE_LEVEL  = None   # overridden at startup by nice=n command-line argument
...
def _short_path(full_path, max_len=25):
    """Truncate path to max_len, showing filename and parent dir."""
    if not full_path:
        return "—"
    p = Path(full_path)
    name = p.name
    ...
    if len(parent) + len(name) + 1 <= max_len:
...
class FilePicker(tk.Frame):
    def __init__(self, parent, label, filetypes, initial_dir=None, **kw):
        ...
        # Browse button right-aligned; entry fills remaining space uniformly
        browse_btn = styled_button(row, " BROWSE", self._browse, small=True)
        browse_btn.pack(side="right", padx=(6, 0))
        self.entry = styled_entry(row, textvariable=self.var, width=10)
...
class IntroBuilderApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(tb, text=f"v{APP_VERSION}", font=("Courier New", 8),
        ...
        # ── Main content area: left (tabs) + right (preview) ──────────────
        main_area = tk.Frame(self, bg=BG)
        main_area.pack(fill="both", expand=True, padx=6, pady=(4, 0))
        main_area.columnconfigure(0, weight=1)  # left (tabs) expands
        main_area.columnconfigure(1, weight=0)  # right (preview) fixed width
        main_area.rowconfigure(0, weight=1)     # tabs+preview row expands

        # ── Notebook tabs (left side) ──────────────────────────────────
        nb = ttk.Notebook(main_area)
        nb.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
        self._notebook = nb

        t1, c1 = self._make_scrollable_tab(nb, "Sources")
        t2, c2 = self._make_scrollable_tab(nb, "Text & Overlays")
        t3, c3 = self._make_scrollable_tab(nb, "Video")
        t4, c4 = self._make_scrollable_tab(nb, "Colour")
        t5, c5 = self._make_scrollable_tab(nb, "Audio")
        t6, c6 = self._make_scrollable_tab(nb, "Duration & Output")
        self._tab_canvases = [c1, c2, c3, c4, c5, c6]

        # ── Preview panel (right side) — built after form so widgets exist ─
        self._build_preview_panel(main_area)

        # ── Build form controls ────────────────────────────────────────
        self._build_form(t1, t2, t3, t4, t5, t6)

        # ── Connect preview traces (after form is built) ────────────────
        self._connect_preview_traces()
        self._update_preview_info()
        self._sync_scrubber_range()

        # Graphic overlay position vars (used by preview handles)
        self.graphic_cx_var = tk.IntVar(value=960)
        self.graphic_cy_var = tk.IntVar(value=540)
        self.graphic_w_var  = tk.IntVar(value=1920)
        self.graphic_h_var  = tk.IntVar(value=1080)
    ...
    def _build_preview_panel(self, parent):
        """Build the interactive preview panel on the right side."""
        PW, PH = 340, 192          # canvas pixel size (16:9)
        self._prev_w, self._prev_h = PW, PH

        prev_frame = tk.Frame(parent, bg=CARD, width=360)
        prev_frame.grid(row=0, column=1, sticky="ns", padx=(0, 0), pady=0)
        prev_frame.pack_propagate(False)

        # Header
        phdr = tk.Frame(prev_frame, bg=CARD)
        phdr.pack(fill="x", padx=10, pady=(10, 4))
        tk.Label(phdr, text="PREVIEW", font=("Courier New", 10, "bold"),
                 fg=ACCENT, bg=CARD).pack(side="left")
        self._prev_time_lbl = tk.Label(phdr, text="t=0.0s",
                 font=("Courier New", 8), fg=SUBTEXT, bg=CARD)
        self._prev_time_lbl.pack(side="right")

        # Canvas
        self._preview_canvas = tk.Canvas(
            prev_frame, width=PW, height=PH,
            bg="#111111", highlightthickness=1, highlightbackground=BORDER,
            cursor="crosshair")
        self._preview_canvas.pack(padx=10, pady=(0, 4))

        # Frame-time scrubber
        scrub_row = tk.Frame(prev_frame, bg=CARD)
        scrub_row.pack(fill="x", padx=10, pady=(0, 2))
        styled_label(scrub_row, "Frame:", color=SUBTEXT, size=8).pack(side="left")
        self._scrub_var = tk.DoubleVar(value=0.0)
        self._scrub_scale = tk.Scale(
            scrub_row, from_=0.0, to=8.0, resolution=0.1,
            orient="horizontal", variable=self._scrub_var,
            showvalue=False, length=230,
            bg=CARD, fg=TEXT, troughcolor=ENTRY_BG,
            highlightbackground=CARD, activebackground=ACCENT, sliderlength=12,
            command=self._on_scrub)
        self._scrub_scale.pack(side="left", fill="x", expand=True)

        # Lock aspect ratio checkbox for fill image handle
        lock_row = tk.Frame(prev_frame, bg=CARD)
        lock_row.pack(fill="x", padx=10, pady=(0, 4))
        self._lock_ar_var = tk.BooleanVar(value=False)
        tk.Checkbutton(lock_row, variable=self._lock_ar_var,
                       bg=CARD, fg=ACCENT, selectcolor=ENTRY_BG,
                       activebackground=CARD, cursor="hand2"
                       ).pack(side="left")
        styled_label(lock_row, "Lock overlay image aspect ratio",
                     color=SUBTEXT, size=8).pack(side="left")

        # Refresh button
        tk.Button(prev_frame, text="↻ Refresh Preview", bg=SURFACE, fg=ACCENT,
        ...
        # Divider
        tk.Frame(prev_frame, bg=BORDER, height=1).pack(fill="x", pady=(2, 6))

        # Info panel
        info_frame = tk.Frame(prev_frame, bg=CARD)
        ...
        for key, label in [
            ...
            row = tk.Frame(info_frame, bg=CARD); row.pack(fill="x", pady=2)
            tk.Label(row, text=f"{label}:", font=("Courier New", 8),
                     fg=SUBTEXT, bg=CARD, width=10, anchor="w").pack(side="left")
            lbl = tk.Label(row, text="—", font=("Courier New", 8),
                           fg=TEXT, bg=CARD)
            lbl.pack(side="left", fill="x", expand=True)
            self._preview_info[key] = lbl

        # Graphic overlay handle state  (canvas coords in preview space)
        self._handle_ids  = []   # canvas item ids for handles
        self._handle_drag = {}   # drag start state
        self._gfx_rect    = [20, 20, PW-20, PH-20]  # [x0,y0,x1,y1] in canvas px
        self._gfx_img_id  = None

        # Bind handle interaction
        self._preview_canvas.bind("<ButtonPress-1>",   self._on_handle_press)
        self._preview_canvas.bind("<B1-Motion>",       self._on_handle_drag)
        self._preview_canvas.bind("<ButtonRelease-1>", self._on_handle_release)
    ...
    def _on_scrub(self, val):
        t = float(val)
        self._prev_time_lbl.config(text=f"t={t:.1f}s")
        # Debounce: cancel pending refresh and re-schedule
        if self._preview_after:
            self.after_cancel(self._preview_after)
        self._preview_after = self.after(400, self._refresh_preview)

    def _sync_scrubber_range(self, *_):
        """Keep scrubber max in sync with duration."""
        try:
            dur = float(self.duration_var.get())
            self._scrub_scale.config(to=max(0.1, dur))
        except Exception:
            pass

    # ── Graphic handles ───────────────────────────────────────────────
    _HANDLE_SIZE = 7  # half-size of corner handle square

    def _draw_handles(self):
        """Draw corner + edge handles on the canvas around _gfx_rect."""
        c = self._preview_canvas
        hs = self._HANDLE_SIZE
        c.delete("handle")
        x0, y0, x1, y1 = self._gfx_rect
        # Draw bounding box
        c.create_rectangle(x0, y0, x1, y1, outline=ACCENT,
                           width=1, dash=(4, 3), tags="handle")
    ...
    def _hit_handle(self, x, y):
        """Return handle tag at (x,y) or None."""
        ...
        for tag, (hx, hy) in positions.items():
            if abs(x - hx) <= hs and abs(y - hy) <= hs:
                return tag
        # Check interior drag (move whole rect)
    ...
    def _on_handle_press(self, event):
        ...
        if hit:
            self._handle_drag = {
                "hit": hit, "sx": event.x, "sy": event.y,
                "rect": list(self._gfx_rect)
            }
        else:
            self._handle_drag = {}
    ...
    def _on_handle_drag(self, event):
        ...
        hit = d["hit"]
        MIN = 20
        lock = getattr(self, "_lock_ar_var", None) and self._lock_ar_var.get()
        orig_w = max(1, x1 - x0)
        orig_h = max(1, y1 - y0)
        ...
        if hit == "move":
            w, h = x1 - x0, y1 - y0
            self._gfx_rect = [x0+dx, y0+dy, x0+dx+w, y0+dy+h]
        else:
            nx0, ny0, nx1, ny1 = x0, y0, x1, y1
            if "n" in hit: ny0 = min(y1 - MIN, y0 + dy)
            if "s" in hit: ny1 = max(y0 + MIN, y1 + dy)
            if "w" in hit: nx0 = min(x1 - MIN, x0 + dx)
            if "e" in hit: nx1 = max(x0 + MIN, x1 + dx)

            if lock:
                nw = nx1 - nx0
                nh = ny1 - ny0
                # Decide which axis drove the resize
                if hit in ("n", "s"):
            ...
            self._gfx_rect = [nx0, ny0, nx1, ny1]

        self._draw_handles()
        self._write_back_handles()
        # Debounced re-render
    ...
    def _write_back_handles(self):
        """Convert canvas rect to 1920x1080 coords and write to form vars."""
        PW, PH = self._prev_w, self._prev_h
        x0, y0, x1, y1 = self._gfx_rect
        sx = 1920 / PW
        sy = 1080 / PH
        # Centre in full-res coords
        cx = int(((x0+x1)/2) * sx)
        cy = int(((y0+y1)/2) * sy)
        w  = int((x1-x0) * sx)
        h  = int((y1-y0) * sy)
        if hasattr(self, "graphic_cx_var"):
            self.graphic_cx_var.set(cx)
        if hasattr(self, "graphic_cy_var"):
            self.graphic_cy_var.set(cy)
        if hasattr(self, "graphic_w_var"):
            self.graphic_w_var.set(w)
        if hasattr(self, "graphic_h_var"):
            self.graphic_h_var.set(h)

    def _rect_from_graphic_vars(self):
        """Compute canvas rect from graphic position vars."""
        PW, PH = self._prev_w, self._prev_h
        sx, sy = PW / 1920, PH / 1080
        if hasattr(self, "graphic_cx_var"):
            cx = self.graphic_cx_var.get() * sx
            cy = self.graphic_cy_var.get() * sy
            w  = self.graphic_w_var.get()  * sx
            h  = self.graphic_h_var.get()  * sy
            self._gfx_rect = [cx-w/2, cy-h/2, cx+w/2, cy+h/2]

    # ── Preview generation (no PIL) ───────────────────────────────────
    def _connect_preview_traces(self):
        self.bg_picker.var.trace_add("write", lambda *_: self._update_preview_info())
        self.audio_picker.var.trace_add("write", lambda *_: self._update_preview_info())
        self.duration_var.trace_add("write", lambda *_: (
            self._update_preview_info(), self._sync_scrubber_range()))
        self.out_name_var.trace_add("write", lambda *_: self._update_preview_info())
        self.out_format_var.trace_add("write", lambda *_: self._update_preview_info())
        if hasattr(self, "_text_widget"):
            self._text_widget.bind("<KeyRelease>", lambda *_: self._update_preview_info())

    def _update_preview_info(self):
        info = {
            "bg_file":    _short_path(self.bg_picker.get(), 25),
            "audio_file": _short_path(self.audio_picker.get(), 25),
            "duration":   f"{self.duration_var.get()}s",
            "out_name":   self.out_name_var.get() or "intro_output",
            "out_format": self.out_format_var.get() or "mp4_h264_aac",
            "text":       (self.text_var.get() or "").split('\n')[0][:30],
        }
        for key, val in info.items():
            if key in self._preview_info:
                self._preview_info[key].config(text=val or "—")

    def _refresh_preview(self):
        """Kick off a background preview render."""
        import threading as _thr
        _thr.Thread(target=self._gen_preview_frame, daemon=True).start()

    def _gen_preview_frame(self):
        """Render a preview frame via ffmpeg. Main graphic is placed within handle rect."""
        try:
            bg = self.bg_picker.get().strip()
            if not bg:
                self._log("Preview: no background selected", SUBTEXT)
                return
            ...
            # Handle rect in canvas coords
            x0, y0, x1, y1 = [int(v) for v in self._gfx_rect]
            gw = max(1, x1 - x0)   # graphic width in canvas px
            gh = max(1, y1 - y0)   # graphic height in canvas px
            gx = x0                 # overlay x in canvas px
            gy = y0                 # overlay y in canvas px

            tmpdir = _tmp.mkdtemp()
            try:
                fill = (self.fill_image_picker.get().strip()
                        if hasattr(self, "fill_image_picker") else "")
                use_fill = (fill and
                            getattr(self, "preserve_ar_var", None) and
                            self.preserve_ar_var.get() and
                            getattr(self, "pad_mode_var", None) and
                            self.pad_mode_var.get() == "image")

                bg_ext = _safe_getext(bg)
                bg_gif = bg_ext == "gif"
                bg_img = bg_ext in ("jpg","jpeg","png","bmp","webp","tiff","gif")

                if bg_gif:
                    bg_flags = ["-ignore_loop", "0", "-i", bg]
                elif bg_img:
                    bg_flags = ["-loop", "1", "-i", bg]
                else:
                    bg_flags = ["-ss", str(t), "-i", bg]

                inter_file = _os.path.join(tmpdir, "inter.png")

                if use_fill:
                    fill_ext = _safe_getext(fill)
                    fill_gif = fill_ext == "gif"
                    fill_img = fill_ext in ("jpg","jpeg","png","bmp","webp","tiff","gif")
                    if fill_gif:
                        fill_flags = ["-ignore_loop", "0", "-i", fill]
                    elif fill_img:
                        fill_flags = ["-loop", "1", "-i", fill]
                    else:
                        # Video fill — seek to 3s for a representative still frame
                        fill_flags = ["-ss", "3", "-i", fill]

                    # Apply chromakey to fill if active
                    ck_active = (getattr(self, "ck_active", None)
                                 and self.ck_active.get())
                    if ck_active:
                        ck_col = (getattr(self, "ck_color_var",
                                  type("_",(),{"get":lambda s:"black"})()).get()
                                  .strip() or "black")
                        ck_tol = float(getattr(self, "ck_tolerance_var",
                                  type("_",(),{"get":lambda s:0.05})()).get())
                        ck_blend = float(getattr(self, "ck_blend_var",
                                         type("_",(),{"get":lambda s:0.15})()).get())
                        fill_proc = (f"format=rgba,colorkey={ck_col}:{ck_tol:.3f}:{ck_blend:.3f},"
                                     f"erosion,"
                                     f"scale={gw}:{gh}:force_original_aspect_ratio=decrease")
                    else:
                        fill_proc = f"scale={gw}:{gh}:force_original_aspect_ratio=decrease"

                    # input 0 = bg (covers full canvas as background)
                    # input 1 = fill (chromakeyed, scaled to handle rect, overlaid on top)
                    filt = (
                        f"[0:v]scale={PW}:{PH}:force_original_aspect_ratio=increase,"
                        f"crop={PW}:{PH}[bg];"
                        f"[1:v]{fill_proc}[overlay];"
                        f"[bg][overlay]overlay={gx}:{gy}[v]"
                    )
                    cmd = (["ffmpeg", "-y"] + bg_flags + fill_flags +
                           ["-filter_complex", filt, "-map", "[v]",
                            "-vframes", "1", inter_file])
                else:
                    # No fill — just scale bg to canvas
                    cmd = (["ffmpeg", "-y"] + bg_flags +
                           ["-vf", f"scale={PW}:{PH}", "-vframes", "1", inter_file])

                r = _sp.run(cmd, capture_output=True, timeout=10)
                if r.returncode != 0 or not _os.path.exists(inter_file):
                    err = r.stderr.decode(errors="replace")[-300:]
                    self._log(f"Preview failed: {err}", DANGER)
                    return

                # PNG → GIF for tk.PhotoImage (no PIL)
                gif_file = _os.path.join(tmpdir, "prev.gif")
                gif_cmd  = ["ffmpeg", "-y", "-i", inter_file,
                            "-filter_complex",
                            "split[a][b];[a]palettegen=stats_mode=single[p];[b][p]paletteuse",
                            gif_file]
                r2 = _sp.run(gif_cmd, capture_output=True, timeout=10)
                load_file = gif_file if (r2.returncode == 0 and
                                          _os.path.exists(gif_file)) else inter_file

                self._tmpdir_ref = tmpdir   # keep alive for tk to read
                self.after(0, self._show_preview_file, load_file)

            except Exception:
                import traceback; traceback.print_exc()

        except Exception as e:
            self._log(f"Preview error: {e}", DANGER)

    def _show_preview_file(self, path):
        """Load image file into canvas (main thread). No PIL needed."""
        try:
            import os as _os
            ...
            PW, PH = self._prev_w, self._prev_h
            if pw > PW or ph > PH:
                s = max(1, max(round(pw/PW), round(ph/PH)))
                photo = photo.subsample(s, s)
            self._show_preview(photo)
        except Exception as e:
            self._log(f"Preview display error: {e}", DANGER)

    def _show_preview(self, photo):
        """Display photo on canvas then redraw handles on top."""
        c = self._preview_canvas
        c.delete("bg_img")
        PW, PH = self._prev_w, self._prev_h
        c.create_image(PW//2, PH//2, image=photo, anchor="center", tags="bg_img")
        c._photo_ref = photo   # prevent GC
        # Lower bg_img below handles
        c.tag_lower("bg_img")
        self._draw_handles()

    # ── Form (split across tab parent frames) ────────────────────
    def _build_form(self, t1, t2, t3, t4, t5, t6):
        pad = dict(padx=20)
        ...
        # BG Start / End — shown only when a video file is selected
        self._bg_range_frame = tk.Frame(fc, bg=CARD)
        _bgr_inner = tk.Frame(self._bg_range_frame, bg=CARD)
        _bgr_inner.pack(fill="x")
        styled_label(_bgr_inner, "BG Start (s):", color=SUBTEXT
                     ).pack(side="left", padx=(0, 6))
        self.seek_var = tk.DoubleVar(value=0.0)
        styled_spinbox(_bgr_inner, self.seek_var, 0.0, 3600.0,
                       width=7, step=0.5).pack(side="left", padx=(0, 24))
        styled_label(_bgr_inner, "BG End (s):", color=SUBTEXT
                     ).pack(side="left", padx=(0, 6))
        self.bg_end_var = tk.DoubleVar(value=0.0)
        _bge_sub = tk.Frame(_bgr_inner, bg=CARD); _bge_sub.pack(side="left")
        styled_spinbox(_bge_sub, self.bg_end_var, 0.0, 3600.0,
                       width=7, step=0.5).pack(side="left")
        styled_label(_bge_sub, "  (0 = full)", color=SUBTEXT, size=8
                     ).pack(side="left")
        # Hidden by default; _on_bg_changed shows it for video files

        # Aspect ratio preserve option (for graphic/image files)
        ar_row = tk.Frame(fc, bg=CARD); ar_row.pack(fill="x", pady=(0, 2))
        self._ar_row_ref = ar_row
        ...
        # Pad mode — radio buttons + colour picker / fill image picker
        # Entire sub-section is stored in _ar_sub so it can be shown/hidden
        self._ar_sub = tk.Frame(fc, bg=CARD, padx=26)

        # Row 1: radio buttons
        pad_mode_row = tk.Frame(self._ar_sub, bg=CARD)
        pad_mode_row.pack(fill="x", pady=(0, 3))
        self.pad_mode_var = tk.StringVar(value="colour")
        styled_label(pad_mode_row, "Padding fill:", color=SUBTEXT
                     ).pack(side="left", padx=(0, 8))
        tk.Radiobutton(pad_mode_row, text="Flat colour",
                       variable=self.pad_mode_var, value="colour",
                       bg=CARD, fg=TEXT, selectcolor=ENTRY_BG,
                       activebackground=CARD, activeforeground=ACCENT,
                       font=FONT_LABEL, cursor="hand2"
                       ).pack(side="left", padx=(0, 12))
        tk.Radiobutton(pad_mode_row, text="Overlay",
                       variable=self.pad_mode_var, value="image",
                       bg=CARD, fg=TEXT, selectcolor=ENTRY_BG,
                       activebackground=CARD, activeforeground=ACCENT,
                       font=FONT_LABEL, cursor="hand2"
                       ).pack(side="left")

        # Row 2a: colour picker (shown in "colour" mode)
        self._pad_col_row = tk.Frame(self._ar_sub, bg=CARD)
        self._pad_col_row.pack(fill="x", pady=(0, 2))
        self._pad_col_lbl = styled_label(self._pad_col_row, "Colour:",
                                         color=SUBTEXT)
        self._pad_col_lbl.pack(side="left", padx=(0, 6))
        self.pad_color_var = tk.StringVar(value="black")
        self._pad_col_btn  = ColourButton(self._pad_col_row, self.pad_color_var,
                     presets=["black", "white", "#111111", "#222222",
                               "gray", "#001020", "#200010"],
                     width=8)
        self._pad_col_btn.pack(side="left")

        # Row 2b: fill image picker (shown in "image" mode)
        self.fill_image_picker = FilePicker(
            self._ar_sub,
            "Overlay image / Video (scaled to cover 1920×1080; main graphic centred on top)",
            [("Images", "*.jpg *.jpeg *.png *.bmp *.webp *.tiff *.tif *.gif"),
             ("Video",  "*.mp4 *.mov *.avi *.mkv *.webm *.m4v *.flv *.wmv"),
             ("All",    "*")],
            initial_dir=str(self.bg_dir) if self.bg_dir.exists() else None)

        def _on_pad_mode(*_):
            if self.pad_mode_var.get() == "colour":
                self._pad_col_row.pack(fill="x", pady=(0, 2))
                self.fill_image_picker.pack_forget()
            else:
                self._pad_col_row.pack_forget()
                self.fill_image_picker.pack(fill="x", pady=(0, 2))
        self.pad_mode_var.trace_add("write", _on_pad_mode)
        self.fill_image_picker.var.trace_add("write", self._update_ck_visibility)

        def _on_preserve_ar(*_):
            if self.preserve_ar_var.get():
                self._ar_sub.pack(fill="x", pady=(0, 4))
            else:
                self._ar_sub.pack_forget()
        self.preserve_ar_var.trace_add("write", _on_preserve_ar)
        # Show sub-section initially (preserve_ar defaults to True)
        self._ar_sub.pack(fill="x", pady=(0, 4))

        # ── Chromakey colour picker ──────────────────────────────────────
        tk.Frame(fc, bg=BORDER, height=1).pack(fill="x", pady=(4, 4))
        # Shown only when the background is a graphic with no alpha channel.
        # The selected colour is treated as transparent via ffmpeg colorkey.
        self._ck_frame = tk.Frame(fc, bg=CARD, padx=8, pady=6)
        # Not packed at construction — shown when a graphic source is selected
        self.ck_active = tk.BooleanVar(value=False)  # tracks whether CK is in use

        ck_hdr = tk.Frame(self._ck_frame, bg=CARD)
        ck_hdr.pack(fill="x", pady=(0, 4))
        tk.Label(ck_hdr,
                 text="\u26a0  No alpha channel detected \u2014 choose a transparency colour",
                 font=("Courier New", 8, "bold"), fg=WARNING, bg=CARD
                 ).pack(side="left")

        ck_row = tk.Frame(self._ck_frame, bg=CARD)
        ck_row.pack(fill="x")
        styled_label(ck_row, "Treat as transparent:", color=SUBTEXT
                     ).pack(side="left", padx=(0, 8))
        ...
        self.ck_color_var = tk.StringVar(value="black")
        ColourButton(ck_row, self.ck_color_var,
                     presets=_CK_PRESETS, width=12
                     ).pack(side="left", padx=(0, 16))

        # Second row: tolerance + blend sliders
        ck_sliders = tk.Frame(self._ck_frame, bg=CARD)
        ck_sliders.pack(fill="x", pady=(4, 0))

        styled_label(ck_sliders, "Tolerance:", color=SUBTEXT
                     ).pack(side="left", padx=(0, 6))
        self.ck_tolerance_var = tk.DoubleVar(value=0.05)
        self._ck_tol_lbl = styled_label(ck_sliders, "0.05", color=ACCENT, size=8)
        tk.Scale(ck_sliders, from_=0.01, to=0.40, resolution=0.01,
                 orient="horizontal", variable=self.ck_tolerance_var,
                 length=100, showvalue=False,
                 bg=CARD, fg=TEXT, troughcolor=ENTRY_BG, sliderlength=14,
                 command=lambda v: self._ck_tol_lbl.config(text=f"{float(v):.2f}")
                 ).pack(side="left", padx=(0, 4))
        self._ck_tol_lbl.pack(side="left")

        styled_label(ck_sliders, "  Blend:", color=SUBTEXT
                     ).pack(side="left", padx=(10, 6))
        self.ck_blend_var = tk.DoubleVar(value=0.15)
        self._ck_blend_lbl = styled_label(ck_sliders, "0.15", color=ACCENT, size=8)
        tk.Scale(ck_sliders, from_=0.0, to=0.50, resolution=0.01,
                 orient="horizontal", variable=self.ck_blend_var,
                 length=100, showvalue=False,
                 bg=CARD, fg=TEXT, troughcolor=ENTRY_BG, sliderlength=14,
                 command=lambda v: self._ck_blend_lbl.config(text=f"{float(v):.2f}")
                 ).pack(side="left", padx=(0, 4))
        self._ck_blend_lbl.pack(side="left")
        ...
        # Track start / end — shown only when "Provided track" is selected
        self._audio_range_row = tk.Frame(fc, bg=CARD, padx=4)
        ...
        def _on_audio_source(*_):
            if self.audio_source_var.get() == "provided":
                self._audio_range_row.pack(fill="x", pady=(4, 0),
                                           after=self.audio_picker)
            else:
        ...
        # ═ DURATION & OUTPUT ══════════════════════════════════════════
        section_header(t6, "DURATION & OUTPUT")
        dc = styled_frame(t6, padx=14, pady=12)
        ...
        # Font row
        tk.Frame(tc, bg=BORDER, height=1).pack(fill="x", pady=(6, 4))
        ...
        # Font preview — full-width row beneath Font/Size/Colour/Border
        _preview_row = tk.Frame(tc, bg=CARD)
        _preview_row.pack(fill="x", pady=(6, 0))
        styled_label(_preview_row, "Font Preview:", color=SUBTEXT).pack(side="left", padx=(0, 8))
        _preview_fallback = avail[0] if avail else "TkDefaultFont"
        _preview_font = self.font_var.get() if self.font_var.get() in avail else _preview_fallback
        self._font_preview_lbl = tk.Label(
            _preview_row, text="Example Text",
            font=(_preview_font, 14),
            fg="#000000", bg="#ffffff",
            padx=10, pady=4, relief="solid", bd=1)
        self._font_preview_lbl.pack(side="left")

        def _update_font_preview(*_):
            chosen = self.font_var.get()
            if chosen in avail:
                try:
                    self._font_preview_lbl.config(font=(chosen, 14))
                except tk.TclError:
                    pass
        self.font_var.trace_add("write", _update_font_preview)
        tk.Frame(tc, bg=BORDER, height=1).pack(fill="x", pady=(6, 4))
        po_row = tk.Frame(tc, bg=CARD); po_row.pack(fill="x", pady=(0, 0))
        ...
        tk.Frame(tc, bg=BORDER, height=1).pack(fill="x", pady=(6, 4))
        tf_hdr = tk.Frame(tc, bg=CARD); tf_hdr.pack(fill="x", pady=(0, 0))
        ...
        # 4-slider grid  (2 columns x 2 rows) — shown only when Text Fade enabled
        tf_grid = tk.Frame(tc, bg=CARD)
        def _on_text_fade_toggle(*_):
            if self.text_fade_enable.get():
                tf_grid.pack(fill="x", pady=(4, 0), after=tf_hdr)
            else:
                tf_grid.pack_forget()
        self.text_fade_enable.trace_add("write", _on_text_fade_toggle)
        # Hidden by default (text_fade_enable defaults to False)
        ...
        lt_f = tk.Frame(tc, bg=CARD, padx=26)
        def _on_lt_toggle(*_):
            if self.ov_lowerthird.get():
                lt_f.pack(fill="x", pady=(2, 6))
            else:
                lt_f.pack_forget()
        self.ov_lowerthird.trace_add("write", _on_lt_toggle)
        # Hidden by default (ov_lowerthird defaults to False)
        ...
        tk.Frame(tc, bg=BORDER, height=1).pack(fill="x", pady=(4, 4))
        ...
        cd_opts = tk.Frame(tc, bg=CARD, padx=26)
        def _on_cd_toggle(*_):
            if self.ov_countdown.get():
                cd_opts.pack(fill="x", pady=(0, 4))
            else:
                cd_opts.pack_forget()
        self.ov_countdown.trace_add("write", _on_cd_toggle)
        # Hidden by default (ov_countdown defaults to False)
        ...
        # Extras opacity
        tk.Frame(tc, bg=BORDER, height=1).pack(fill="x", pady=(4, 4))
        eo_row = tk.Frame(tc, bg=CARD); eo_row.pack(fill="x", pady=(0, 0))
        ...
        # ── helper: one row = checkbox  label/desc  |  slider (when has intensity) ──
        def _fx_row(parent, label, desc, has_slider=False,
                    sl_var=None, sl_lo=0.0, sl_hi=1.0,
                    sl_res=0.05, sl_fmt="{:.2f}", sl_len=180, default=False):
            tk.Frame(parent, bg=BORDER, height=1).pack(fill="x", pady=(4, 0))
            row = tk.Frame(parent, bg=CARD)
            row.pack(fill="x", pady=1)

            # Left: checkbox + label block (fixed width so sliders align)
            left = tk.Frame(row, bg=CARD, width=200)
            left.pack(side="left", fill="y")
            left.pack_propagate(False)

            cb_var = tk.BooleanVar(value=default)
            tk.Checkbutton(left, variable=cb_var, bg=CARD,
                           activebackground=CARD, selectcolor=ENTRY_BG,
                           fg=ACCENT, relief="flat", cursor="hand2"
                           ).pack(side="left", padx=(4, 0))
            col = tk.Frame(left, bg=CARD); col.pack(side="left")
            tk.Label(col, text=label, font=("Courier New", 9, "bold"),
                     fg=TEXT, bg=CARD).pack(anchor="w")
            if desc:
                tk.Label(col, text=desc, font=("Courier New", 8),
            ...
            # Right: intensity slider (only when has_slider)
            if has_slider and sl_var is not None:
                right = tk.Frame(row, bg=CARD)
                right.pack(side="left", fill="x", expand=True, padx=(8, 0))
                top = tk.Frame(right, bg=CARD); top.pack(fill="x")
                styled_label(top, "Intensity", color=SUBTEXT, size=8).pack(side="left")
                val_lbl = styled_label(top, sl_fmt.format(sl_var.get()),
                                       color=ACCENT, size=8)
                val_lbl.pack(side="right")
                tk.Scale(right, from_=sl_lo, to=sl_hi, resolution=sl_res,
                         orient="horizontal", variable=sl_var,
                         length=sl_len, showvalue=False,
                         bg=CARD, fg=TEXT, troughcolor=ENTRY_BG,
                         highlightbackground=CARD, activebackground=ACCENT,
                         sliderlength=14,
                         command=lambda v, lbl=val_lbl, fmt=sl_fmt:
                             lbl.config(text=fmt.format(float(v)))
                         ).pack(fill="x")
            else:
                # No slider — add a spacer so the row height matches slider rows
                tk.Frame(row, bg=CARD, height=44).pack(side="left")

            # Return a minimal object matching EffectToggle interface
            class _Tog:
        ...
        # ── intensity vars ──────────────────────────────────────────────
        self.grain_intensity_var    = tk.DoubleVar(value=12.0)
        self.vignette_intensity_var = tk.DoubleVar(value=30.0)
        self.sharpen_intensity_var  = tk.DoubleVar(value=1.0)
        self.blur_intensity_var     = tk.IntVar(value=3)
        self.bloom_intensity_var    = tk.DoubleVar(value=0.4)
        self.scan_intensity_var     = tk.DoubleVar(value=0.18)
        self.glitch_intensity_var   = tk.DoubleVar(value=0.7)
        self.flash_intensity_var    = tk.DoubleVar(value=0.15)
        self.pulse_intensity_var    = tk.IntVar(value=60)
        self.drift_intensity_var    = tk.IntVar(value=156)

        # ── effect rows ─────────────────────────────────────────────────
        self.fx_scan      = _fx_row(vfx, "Scanline Sweep", "Animated light bar",
                                    True, self.scan_intensity_var,
                                    0.02, 0.6, 0.01, "{:.2f}")
        self.fx_drift     = _fx_row(vfx, "Slow Drift",     "Pan gently left to right",
                                    True, self.drift_intensity_var,
                                    20, 300, 10, "{:.0f}px", 180, False)
        self.fx_pulse     = _fx_row(vfx, "Pulse",          "Rhythmic breathe / zoom",
                                    True, self.pulse_intensity_var,
                                    10, 120, 5, "{:.0f}px", 180, False)
        self.fx_grain     = _fx_row(vfx, "Film Grain",     "Analogue noise overlay",
                                    True, self.grain_intensity_var,
                                    1.0, 50.0, 0.5, "{:.1f}")
        self.fx_vignette  = _fx_row(vfx, "Vignette",       "Darkened edges",
                                    True, self.vignette_intensity_var,
                                    5.0, 90.0, 1.0, "{:.0f}°")
        self.fx_sharpen   = _fx_row(vfx, "Sharpen",        "Crisp edge detail",
                                    True, self.sharpen_intensity_var,
                                    0.1, 3.0, 0.1, "{:.1f}")
        self.fx_blur      = _fx_row(vfx, "Blur",           "Gaussian blur",
                                    True, self.blur_intensity_var,
                                    1, 20, 1, "{:.0f}")
        self.fx_bloom     = _fx_row(vfx, "Bloom",          "Soft glow on highlights",
                                    True, self.bloom_intensity_var,
                                    0.05, 1.0, 0.05, "{:.2f}")
        self.fx_flash     = _fx_row(vfx, "Flash Bar",      "Mid-video white flash",
                                    True, self.flash_intensity_var,
                                    0.05, 0.6, 0.01, "{:.2f}")
        self.fx_glitch    = _fx_row(vfx, "Glitch",         "Digital corruption flicker",
                                    True, self.glitch_intensity_var,
                                    0.1, 1.0, 0.05, "{:.2f}")
        # Mirror and Letterbox at bottom — no intensity slider
        self.fx_mirror    = _fx_row(vfx, "Mirror Tiles",   "2×2 kaleidoscope")
        self.fx_letterbox = _fx_row(vfx, "Letterbox",      "Cinematic 2.35:1 bars")

        # Reset button
        vfx_reset_row = tk.Frame(t3, bg=BG)
        vfx_reset_row.pack(fill="x", padx=20, pady=(0, 4))
        ...
        # ═ COLOUR OPTIONS ═════════════════════════════════════════════
        section_header(t4, "COLOUR OPTIONS")
        co_card = styled_frame(t4, padx=14, pady=12)
        co_card.pack(fill="x", **pad, pady=(0, 4))

        # ── Sepia / B&W ─────────────────────────────────────────────────
        tk.Frame(co_card, bg=BORDER, height=1).pack(fill="x", pady=(0, 4))
        ...
        for m in ("sepia", "black & white"):
            tk.Radiobutton(sep_row, text=m, variable=self.sepia_mode_var, value=m,
                           bg=CARD, fg=TEXT, selectcolor=ENTRY_BG,
                           activebackground=CARD, font=FONT_LABEL, cursor="hand2"
        ...
        # Fade duration — shown only when Fade to Colour is enabled
        fade_dur_row = tk.Frame(co_card, bg=CARD, padx=4)
        def _on_fade_col_toggle(*_):
            if self.fx_fade_col_var.get():
                fade_dur_row.pack(fill="x", pady=(6, 0))
            else:
                fade_dur_row.pack_forget()
        self.fx_fade_col_var.trace_add("write", _on_fade_col_toggle)
        # Packed initially because fx_fade_col_var defaults to True
        ...
        fade_dur_row.pack(fill="x", pady=(6, 0))   # default: Fade to Colour starts enabled


        # ── Colour Grading — same alignment as Fade to Colour ─────────────
        tk.Frame(co_card, bg=BORDER, height=1).pack(fill="x", pady=(4, 4))
        ...
        # ═ AUDIO EFFECTS ══════════════════════════════════════════════
        section_header(t5, "AUDIO EFFECTS")
        afx_card = styled_frame(t5, padx=14, pady=10)
        afx_card.pack(fill="x", **pad, pady=(0, 4))

        # ── Master volume (always visible, no checkbox) ───────────────
        ...
        # ── helper: one audio row = checkbox | label/desc | sliders ──
        def _afx_row(label, desc, sliders=None, default=False):
            """sliders: list of (attr, label, var, lo, hi, res, fmt)"""
            tk.Frame(afx_card, bg=BORDER, height=1).pack(fill="x", pady=(4, 0))
            row = tk.Frame(afx_card, bg=CARD)
            row.pack(fill="x", pady=1)
            ...
            if sliders:
                right = tk.Frame(row, bg=CARD)
                right.pack(side="left", fill="x", expand=True, padx=(8, 0))
                for (attr, sl_label, var, lo, hi, res, fmt) in sliders:
                    sr = tk.Frame(right, bg=CARD); sr.pack(fill="x", pady=(1, 0))
                    top = tk.Frame(sr, bg=CARD); top.pack(fill="x")
                    styled_label(top, sl_label, color=SUBTEXT, size=8).pack(side="left")
                    val_lbl = styled_label(top, fmt.format(var.get()), color=ACCENT, size=8)
                    val_lbl.pack(side="right")
                    tk.Scale(sr, from_=lo, to=hi, resolution=res,
                             orient="horizontal", variable=var,
                             length=180, showvalue=False,
                             bg=CARD, fg=TEXT, troughcolor=ENTRY_BG,
                             highlightbackground=CARD, activebackground=ACCENT,
                             sliderlength=14,
                             command=lambda v, lbl=val_lbl, f=fmt:
        ...
        self.afx_fade = _afx_row(
            "Fade In / Out", "Crossfade",
            [("fade_dur_var", "Fade duration (s)", self.fade_dur_var, 0.1, 4.0, 0.1, "{:.1f}")],
            default=True)
        self.afx_bass = _afx_row(
            "Bass Punch", "Low-frequency EQ boost",
            [("bass_freq_var", "Freq (Hz)",  self.bass_freq_var,  30,   300,  5,    "{:.0f}"),
             ("bass_gain_var", "Gain (dB)",  self.bass_gain_var,  0.0,  18.0, 0.1,  "{:.1f}")])
        self.afx_reverb = _afx_row(
            "Reverb", "Hall reverb (aecho)",
            [("reverb_delay_ms", "Delay (ms)", self.reverb_delay_ms, 10, 200, 5,   "{:.0f}"),
             ("reverb_decay",    "Decay",      self.reverb_decay,    0.05, 1.0, 0.05, "{:.2f}")])
        self.afx_compressor = _afx_row(
            "Compressor", "Dynamic range control",
            [("comp_thr_var",  "Threshold (dB)", self.comp_thr_var,  -40.0,  0.0, 0.5, "{:.1f}"),
             ("comp_ratio_var","Ratio",           self.comp_ratio_var, 1.0,  20.0, 0.5, "{:.1f}")])
        self.afx_echo = _afx_row(
            "Echo / Delay", "Slap-back echo",
            [("echo_delay_ms",  "Delay (ms)", self.echo_delay_ms,  10, 500, 5,    "{:.0f}"),
             ("echo_decay_var", "Decay",      self.echo_decay_var, 0.1, 0.95, 0.05, "{:.2f}")])
        self.afx_highpass = _afx_row(
            "High-Pass Filter", "Remove rumble",
            [("hp_freq_var", "Cutoff (Hz)", self.hp_freq_var, 20, 800, 10, "{:.0f}")])
        self.afx_lowpass = _afx_row(
            "Low-Pass Filter", "Soften highs",
            [("lp_freq_var", "Cutoff (Hz)", self.lp_freq_var, 1000, 20000, 100, "{:.0f}")])
        self.afx_tremolo = _afx_row(
            "Tremolo", "Rhythmic volume pulse",
            [("tremolo_rate",  "Rate (Hz)", self.tremolo_rate,  0.5, 20.0, 0.5, "{:.1f}"),
             ("tremolo_depth", "Depth",     self.tremolo_depth, 0.0,  1.0, 0.05, "{:.2f}")])

        # ── effects without extra controls (bottom of column) ─────────
        self.afx_wide      = _afx_row("Stereo Widener", "Wider soundstage")
        self.afx_normalize = _afx_row("Normalize",      "Auto loudness to -16 LUFS")
        self.afx_chorus    = _afx_row("Chorus",         "Rich chorus shimmer")

        # Reset button
        reset_row = tk.Frame(afx_card, bg=CARD)
        ...
        # ═ QUALITY ════════════════════════════════════════════════════
        section_header(t6, "QUALITY")
        qc = styled_frame(t6, padx=14, pady=12)
    ...
    def _get_settings_dict(self):
        return {
            ...
            "fx_blur":        getattr(self,"fx_blur",type("_",(),{"var":type("_",(),{"get":lambda s:False})()})()).var.get(),
            "blur_intensity": int(getattr(self,"blur_intensity_var",type("_",(),{"get":lambda s:3})()).get()),
            "grain_intensity":    float(getattr(self,"grain_intensity_var",type("_",(),{"get":lambda s:12.0})()).get()),
            "vignette_intensity": float(getattr(self,"vignette_intensity_var",type("_",(),{"get":lambda s:30.0})()).get()),
            "sharpen_intensity":  float(getattr(self,"sharpen_intensity_var",type("_",(),{"get":lambda s:1.0})()).get()),
            "bloom_intensity":    float(getattr(self,"bloom_intensity_var",type("_",(),{"get":lambda s:0.4})()).get()),
            "scan_intensity":     float(getattr(self,"scan_intensity_var",type("_",(),{"get":lambda s:0.18})()).get()),
            "glitch_intensity":   float(getattr(self,"glitch_intensity_var",type("_",(),{"get":lambda s:0.7})()).get()),
            "flash_intensity":    float(getattr(self,"flash_intensity_var",type("_",(),{"get":lambda s:0.15})()).get()),
            "pulse_intensity":    int(getattr(self,"pulse_intensity_var",type("_",(),{"get":lambda s:60})()).get()),
            "drift_intensity":    int(getattr(self,"drift_intensity_var",type("_",(),{"get":lambda s:156})()).get()),
            ...
            "pad_mode":       getattr(self, "pad_mode_var",
                                      type("_",(),{"get":lambda s:"colour"})()).get(),
            "fill_image":     getattr(self, "fill_image_picker",
            ...
            "ck_color":       getattr(self, "ck_color_var",
                                      type("_",(),{"get":lambda s:"black"})()).get(),
            "ck_tolerance":   float(getattr(self, "ck_tolerance_var",
                                            type("_",(),{"get":lambda s:0.05})()).get()),
            "ck_blend":       float(getattr(self, "ck_blend_var",
                                            type("_",(),{"get":lambda s:0.15})()).get()),
            "ck_active":      self.ck_active.get(),
            "graphic_cx":     self.graphic_cx_var.get(),
            "graphic_cy":     self.graphic_cy_var.get(),
            "graphic_w":      self.graphic_w_var.get(),
            "graphic_h":      self.graphic_h_var.get(),
            "out_name":       self.out_name_var.get(),
            "lock_ar":        getattr(self, "_lock_ar_var",
                                      type("_",(),{"get":lambda s:False})()).get(),
        }
    ...
    def _apply_settings_dict(self, d):
        simple_vars = {
            ...
            "blur_intensity":     getattr(self,"blur_intensity_var",tk.IntVar()),
            "grain_intensity":    getattr(self,"grain_intensity_var",tk.DoubleVar()),
            "vignette_intensity": getattr(self,"vignette_intensity_var",tk.DoubleVar()),
            "sharpen_intensity":  getattr(self,"sharpen_intensity_var",tk.DoubleVar()),
            "bloom_intensity":    getattr(self,"bloom_intensity_var",tk.DoubleVar()),
            "scan_intensity":     getattr(self,"scan_intensity_var",tk.DoubleVar()),
            "glitch_intensity":   getattr(self,"glitch_intensity_var",tk.DoubleVar()),
            "flash_intensity":    getattr(self,"flash_intensity_var",tk.DoubleVar()),
            "pulse_intensity":    getattr(self,"pulse_intensity_var",tk.IntVar()),
            "drift_intensity":    getattr(self,"drift_intensity_var",tk.IntVar()),
            "pad_color":  self.pad_color_var,
            "pad_mode":   getattr(self, "pad_mode_var", tk.StringVar()),
            "audio_seek": self.audio_seek_var,
            "audio_end":  self.audio_end_var,
            "ck_color":       getattr(self, "ck_color_var", tk.StringVar()),
            "ck_tolerance":   getattr(self, "ck_tolerance_var", tk.DoubleVar()),
            "ck_blend":       getattr(self, "ck_blend_var", tk.DoubleVar()),
            "ck_active":      self.ck_active,
            "graphic_cx":     self.graphic_cx_var,
            "graphic_cy":     self.graphic_cy_var,
            "graphic_w":      self.graphic_w_var,
            "graphic_h":      self.graphic_h_var,
            "out_name":       self.out_name_var,
            "lock_ar":        getattr(self, "_lock_ar_var", tk.BooleanVar()),
        }
        bool_vars = {
        ...
        toggles = {
            ...
            "fx_blur":      getattr(self,"fx_blur",type("_",(),{"var":tk.BooleanVar()})()),
        ...
        if "fill_image" in d and d["fill_image"]:
            try:
                if getattr(self, "fill_image_picker", None):
                    self.fill_image_picker.set(d["fill_image"])
            except Exception: pass
    ...
    def _on_bg_changed(self, *_):
        ...
        if is_img:
            ...
            if hasattr(self, "_bg_range_frame"):
                self._bg_range_frame.pack_forget()
        else:
            self._rb_bg.config(state="normal")
            self._rb_prov.config(state="normal")
            self._rb_silent.config(state="normal")
            if hasattr(self, "_bg_range_frame") and hasattr(self, "_ar_row_ref"):
                self._bg_range_frame.pack(fill="x", pady=(0, 6),
                                          before=self._ar_row_ref)
            import threading as _thr
            _thr.Thread(target=self._probe_bg_audio, args=(path,),
                        daemon=True).start()
        self._update_ck_visibility()

    def _update_ck_visibility(self, *_):
        """Show chromakey panel directly under fill_image_picker when fill is set,
        or under bg_picker when bg is a static image with no fill."""
        bg_path   = self.bg_picker.get().strip()
        bg_ext    = _safe_getext(bg_path)
        bg_is_img = bg_ext in ("jpg","jpeg","png","bmp","webp","tiff","gif")
        fill_path = (getattr(self, "fill_image_picker", None) and
                     self.fill_image_picker.get().strip())
        show = bg_is_img or bool(fill_path)
        if show:
            # Pack after fill_image_picker if available, otherwise after bg_picker
            anchor = (self.fill_image_picker
                      if (fill_path and hasattr(self, "fill_image_picker"))
                      else self.bg_picker)
            self._ck_frame.pack(fill="x", pady=(4, 2), after=anchor)
            self.ck_active.set(True)
        else:
            self._ck_frame.pack_forget()
            self.ck_active.set(False)
    ...
    def _reset_video_effects(self):
        """Reset all video effects (including Colour Options effects) to off."""
        for tog in (self.fx_eq, self.fx_scan, self.fx_drift, self.fx_mirror,
                    ...
                    self.fx_sharpen, self.fx_blur):
            tog.var.set(False)
        # Reset intensity sliders to defaults
        defaults = {
            "blur_intensity_var":     3,
            "grain_intensity_var":    12.0,
            "vignette_intensity_var": 30.0,
            "sharpen_intensity_var":  1.0,
            "bloom_intensity_var":    0.4,
            "scan_intensity_var":     0.18,
            "glitch_intensity_var":   0.7,
            "flash_intensity_var":    0.15,
            "pulse_intensity_var":    60,
            "drift_intensity_var":    156,
        }
        for attr, val in defaults.items():
            if hasattr(self, attr):
                getattr(self, attr).set(val)
    ...
    def _start_build(self):
        ...
        # Validate fill image when image pad mode is active
        if (self.preserve_ar_var.get()
                and getattr(self, "pad_mode_var", None)
                and self.pad_mode_var.get() == "image"):
            _fill = getattr(self, "fill_image_picker", None)
            _fill_path = _fill.get() if _fill else ""
            if not _fill_path or not os.path.isfile(_fill_path):
                messagebox.showwarning(
                    "Missing Fill Image",
                    "Select a fill/background image for the padding area,\n"
                    "or switch Padding fill back to 'Flat colour'.")
                return
        ...
        try:
            ...
            if vf_bg == "__MIRROR__":
                ...
                cmd1 += ["-i", self.bg_picker.get()] + _bg_t_arg + [
                         ...
                         self.tmp_bg]
            elif vf_bg == "__FILL_IMAGE__":
                fill_path = self.fill_image_picker.get()
                fill_fc   = self._build_fill_image_fc(dur)
                self._log(f"  fill_fc : {fill_fc}", WARNING)
                out_dur = ["-to", str(bg_end)] if bg_end > 0.0 else ["-t", str(dur)]
                # Main graphic (bg): input 0
                if ext == "gif" and is_animated:
                    main_in_flags = ["-ignore_loop", "0", "-i", self.bg_picker.get()]
                elif is_image:
                    main_in_flags = ["-loop", "1", "-i", self.bg_picker.get()]
                else:
                    seek_s = float(getattr(self, "seek_var",
                                   type("_",(),{"get":lambda s:0.0})()).get())
                    main_in_flags = (["-ss", str(seek_s)] if seek_s > 0 else []) + \
                                    ["-stream_loop", "-1", "-i", self.bg_picker.get()]
                # Fill image: input 1
                fill_ext      = _safe_getext(fill_path)
                fill_is_gif   = fill_ext == "gif"
                fill_is_image = fill_ext in ("jpg","jpeg","png","bmp","webp","tiff","gif")
                if fill_is_gif:
                    fill_in_flags = ["-ignore_loop", "0", "-i", fill_path]
                elif fill_is_image:
                    fill_in_flags = ["-loop", "1", "-i", fill_path]
                else:
                    fill_in_flags = ["-stream_loop", "-1", "-i", fill_path]
                # Duration goes on the OUTPUT side so both looping inputs are cut
                cmd1 = (["ffmpeg", "-y", "-threads", "2"]
                        + main_in_flags          # input 0: bg
                        + fill_in_flags          # input 1: fill overlay
                        + ["-filter_complex", fill_fc,
                           "-map", "[vout]"]
                        + out_dur                # output duration limit
                        + ["-c:v", "libx264", "-preset", "ultrafast", "-crf", "18",
                           "-threads", "2", "-an", "-movflags", "+faststart",
                           self.tmp_bg])
            else:
                ...
                cmd1 += ["-i", self.bg_picker.get()] + _bg_t_arg + [
                         ...
                         "-movflags", "+faststart",
                         self.tmp_bg]
            self._run_ffmpeg(cmd1, "bg", progress_range=(0, 30))
            ...
            if fmt!="gif_animated":
                ...
                self._run_ffmpeg(cmd3, "composite", progress_range=(60, 95))
            else:
                ...
                self._run_ffmpeg(cmd3, "composite_gif_mid", progress_range=(60, 80))
                palette=str(self.base_dir/(base_out+".__pal__.png"))
                gif_fps=12
                self._run_ffmpeg(["ffmpeg","-y","-i",mid_mp4,"-vf",f"fps={gif_fps},scale=iw:-1:flags=lanczos,palettegen",palette], "gif_palette", progress_range=(80, 85))
                self._run_ffmpeg(["ffmpeg","-y","-i",mid_mp4,"-i",palette,"-lavfi",f"fps={gif_fps},scale=iw:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=sierra2_4a","-loop","0",out_path], "gif_encode", progress_range=(85, 95))
    ...
    # ── Fill-image filter_complex builder ────────────────────────────
    def _build_fill_image_fc(self, dur):
        """
        Build a filter_complex string for fill-image padding mode.

        Input 0 : main graphic (bg_picker) — cover-scaled to 1920×1080 as background
        Input 1 : fill image  — chromakeyed, scaled to handle rect size, overlaid on top

        The fill image's position and size come from the preview handle rect vars
        (graphic_cx_var, graphic_cy_var, graphic_w_var, graphic_h_var).
        """
        # Fill image position/size from preview handles (default: centred, full frame)
        cx = int(getattr(self, "graphic_cx_var", type("_",(),{"get":lambda s:960})()).get())
        cy = int(getattr(self, "graphic_cy_var", type("_",(),{"get":lambda s:540})()).get())
        fw = int(getattr(self, "graphic_w_var",  type("_",(),{"get":lambda s:1920})()).get())
        fh = int(getattr(self, "graphic_h_var",  type("_",(),{"get":lambda s:1080})()).get())
        ox = max(0, cx - fw // 2)
        oy = max(0, cy - fh // 2)

        # Chromakey applied to fill image
        ck_active = getattr(self, "ck_active", None) and self.ck_active.get()
        ck_pre = ""
        if ck_active:
            ck_col = (getattr(self, "ck_color_var",
                              type("_", (), {"get": lambda s: "black"})()).get()
            ...
            ck_tol = float(getattr(self, "ck_tolerance_var",
                                   type("_", (), {"get": lambda s: 0.05})()).get())
            # blend=0.15 feathers the key edge to remove green halo;
            # erosion shrinks the matte by 1px to clip residual fringe pixels
            ck_blend = float(getattr(self, "ck_blend_var",
                                     type("_",(),{"get":lambda s:0.15})()).get())
            ck_pre = (f"format=rgba,"
                      f"colorkey={ck_col}:{ck_tol:.3f}:{ck_blend:.3f},"
                      f"erosion,")

        fill_scale = (
            f"fps=30,{ck_pre}scale={fw}:{fh}:force_original_aspect_ratio=decrease"
        )

        # Main graphic scale — always cover full 1920×1080
        src_path = self.bg_picker.get()
        ext      = _safe_getext(src_path)
        is_animated = (ext == "gif" and self._is_animated_gif(src_path))
        ...
        if self.fx_drift.get():
            dpx = int(getattr(self, "drift_intensity_var",
                              type("_",(),{"get":lambda s:156})()).get())
            post += [f"crop=1920:1080:'min(t/{dur}*{dpx},{dpx})':0"]
        if self.fx_pulse.get():
            ppx = int(getattr(self, "pulse_intensity_var",
                              type("_",(),{"get":lambda s:60})()).get())
            half = ppx // 2
            sw = 1920 + ppx * 2; sh = 1080 + ppx * 2
            post += [
                f"scale={sw}:{sh}",
                f"crop=1920:1080"
                f":{ppx}+{half}+{half}*sin(2*PI*t/{max(dur/2,1)})'"
                f":{ppx//2 - ppx//2}+{ppx//4}+{ppx//4}*sin(2*PI*t/{max(dur/2,1)})'",
            ]
        if self.fx_hueshift.get():
            post.append(f"hue=h='mod(t/{dur}*120,360)'")
        if getattr(self, "fx_blur", None) and self.fx_blur.get():
            intensity = max(1, int(getattr(
                self, "blur_intensity_var",
                type("_", (), {"get": lambda s: 3})()).get()))
            post.append(f"gblur=sigma={intensity}")
        ...
        if self.fx_drift.get():
            ...
            bw = 1920 + dpx
            return (
                f"[0:v]scale={bw}:1080:force_original_aspect_ratio=increase,"
                f"crop={bw}:1080[bg];"
                f"[1:v]{fill_scale}[fill];"
                f"[bg][fill]overlay={ox}:{oy}"
                + post_str
                + ",format=yuv420p[vout]"
            )
        else:
    ...
    def _build_bg_vf(self, dur):
        ...
        # Formats that support an alpha channel — need background compositing
        has_alpha = ext in ("png", "webp", "tiff")
        ...
        # Fill image mode — active whenever pad_mode=image and fill path is set,
        # regardless of whether the background is a still image or video.
        pad_mode = getattr(self, "pad_mode_var", None)
        if (pad_mode and pad_mode.get() == "image"
                and getattr(self, "fill_image_picker", None)
                and self.fill_image_picker.get()):
            return "__FILL_IMAGE__"

        preserve = is_img and getattr(self, "preserve_ar_var", None) and self.preserve_ar_var.get()

        self._log(f"  img={is_img}  preserve_ar={preserve}  pad_col={pad_col}  has_alpha={has_alpha}", SUBTEXT)

        parts = []

        # For images that may carry alpha (PNG, WebP, TIFF), flatten against
        # the pad colour BEFORE scaling so transparent areas show the pad
        # colour rather than becoming black in yuv420p conversion.
        # We do this by prefixing "format=rgba" so FFmpeg reads the alpha,
        # then using "alphamix" via the split+overlay approach expressed as
        # a single chain: split the rgba stream, composite onto a solid bg.
        # In practice the cleanest -vf-compatible approach is:
        #   format=rgba → scale (keeps alpha) → pad (fills with pad_col,
        #   but pad_col has no alpha, so transparent regions get pad_col)
        #   → format=yuv420p  (drops alpha, transparent areas = pad_col)
        # This works because FFmpeg's pad filter fills with the given colour
        # at full opacity, which correctly replaces transparent regions when
        # going from rgba → yuv420p.
        if has_alpha:
            parts = ["format=rgba"]
        else:
            parts = []

        # If the chromakey picker is visible the user wants a specific colour
        # treated as transparent on the main graphic.  Apply colorkey before
        # any scaling so it operates at source resolution.
        ck_active = getattr(self, "ck_active", None) and self.ck_active.get()
        if ck_active:
            ck_col = (getattr(self, "ck_color_var",
                              type("_", (), {"get": lambda s: "black"})()).get()
            ...
            ck_tol = float(getattr(self, "ck_tolerance_var",
                                   type("_", (), {"get": lambda s: 0.05})()).get())
            # colorkey needs rgba input; ensure it is set
            if not parts:
                parts = ["format=rgba"]
            parts.append(f"colorkey={ck_col}:{ck_tol:.3f}:0.0")
            # After colorkey the stream has alpha — treat it the same as has_alpha
            has_alpha = True
        ...
        if self.fx_drift.get():
            ...
            if preserve:
                parts += [
                    "fps=30",
                    f"scale={fw}:1080:force_original_aspect_ratio=decrease",
                    f"pad={fw}:1080:(ow-iw)/2:(oh-ih)/2:{pad_col}",
                    f"crop=1920:1080:'min(t/{dur}*{dpx},{dpx})':0",
                ]
            else:
                parts += [
                    "fps=30",
            ...
            if preserve:
                ...
                # Even without preserve_ar, still need to composite alpha
                parts += [
                    "fps=30",
                    f"scale=1920:1080",
                    f"pad=1920:1080:0:0:{pad_col}",
                ]
            else:
        ...
        if getattr(self, "fx_blur", None) and self.fx_blur.get():
            intensity = max(1, int(getattr(self, "blur_intensity_var",
                                           type("_",(),{"get":lambda s:3})()).get()))
            # gblur gives a true Gaussian blur; sigma = intensity
            parts.append(f"gblur=sigma={intensity}")
    ...
    def _build_filter_complex(self, text, dur):
        ...
        if self.fx_vignette.get():
            angle = getattr(self, "vignette_intensity_var", None)
            a = float(angle.get()) if angle else 30.0
            pre.append(f"vignette=PI/{max(1, int(180/a))}")
        if self.fx_grain.get():
            g = float(getattr(self, "grain_intensity_var",
                              type("_",(),{"get":lambda s:12.0})()).get())
            pre.append(f"noise=alls={g:.1f}:allf=t+u")
        if self.fx_sharpen.get():
            s = float(getattr(self, "sharpen_intensity_var",
                              type("_",(),{"get":lambda s:1.0})()).get())
            pre.append(f"unsharp=5:5:{s:.2f}:5:5:0.0")
        ...
        if self.fx_bloom.get():
            bo = float(getattr(self, "bloom_intensity_var",
                               type("_",(),{"get":lambda s:0.4})()).get())
            ...
            return (f"[0:v]{pre_s}split[ba][bb];"
                    ...
                    f"[ba][bg]blend=all_mode=screen:all_opacity={bo:.2f}"
                    f"{post_s}[v]")
        else:
            return "[0:v]" + ",".join(all_filters) + "[v]"

    # ── ffmpeg runner ─────────────────────────────────────────────────
    def _run_ffmpeg(self, cmd, label, progress_range=(0, 100), expected_duration=None):
        """Run ffmpeg, parsing stderr char-by-char for smooth real-time progress.

        progress_range : (start_pct, end_pct) slice of the 0-100 bar this step owns.
        expected_duration : seconds; falls back to duration_var.
        """
        self._log("  $ " + " ".join(cmd[:6]) + " ...", SUBTEXT)
        p0, p1 = progress_range
        dur = float(expected_duration if expected_duration is not None
                    else self.duration_var.get())
        self._set_progress(p0)

        run_cmd = cmd
        if NICE_LEVEL is not None and sys.platform.startswith("linux"):
            run_cmd = ["nice", "-n", str(NICE_LEVEL)] + cmd

        try:
            proc = subprocess.Popen(
                run_cmd,
                stdout=subprocess.PIPE,   # ffmpeg info/warnings → log
                stderr=subprocess.PIPE,   # ffmpeg progress → progress bar
                )

            # ── stderr reader: character-by-character so \r lines are caught ──
            _last_progress_line = [""]
            _last_log_time      = [0.0]   # last logged time value
            _LOG_INTERVAL       = 1.0     # log at most once per second
            ...
            def _read_stderr():
                ...
                while True:
                    ...
                    if ch in ("\r", "\n"):
                        ...
                        if not line:
                            continue
                        # Parse time= and fps= from progress line
                        m_time = re.search(r"time=(\d+):(\d+):([\d.]+)", line)
                        m_fps  = re.search(r"fps=\s*([\d.]+)", line)
                        if m_time and dur > 0:
                            h   = int(m_time.group(1))
                            mn  = int(m_time.group(2))
                            s   = float(m_time.group(3))
                            cur = h * 3600 + mn * 60 + s
                            frac = min(1.0, cur / dur)
                            self._ui(lambda f=frac: self._set_progress(
                                p0 + f * (p1 - p0)))
                            # Log time + fps at most once per second
                            now = _time.monotonic()
                            if now - _last_log_time[0] >= _LOG_INTERVAL:
                                _last_log_time[0] = now
                                fps_str = ""
                                if m_fps:
                                    fps_str = f"  fps={float(m_fps.group(1)):.1f}"
                                self._log(
                                    f"  time={cur:.1f}s / {dur:.1f}s"
                                    f"{fps_str}", "#334455")
                        # Always log errors
            ...
            if rc != 0:
                # Surface the last stderr line as the error message
                raise RuntimeError(
                    f"ffmpeg failed for '{label}' (exit {rc}): "
                    f"{_last_progress_line[0][-200:]}")
        except FileNotFoundError:
            raise RuntimeError(
                "ffmpeg not found. Install: sudo apt install ffmpeg")

    def _find_font(self, name):
        """Locate a font file by name.
        Priority:
          1. fc-match (fontconfig) — the proper system font resolver; handles
             aliases, variants, and installed fonts correctly.
          2. Directory scan — fallback for systems without fontconfig.
        ...
        try:
            ...
            if path and Path(path).exists():
                # fc-match always returns *something* — verify it actually
                # matches the requested name (not just the system fallback)
                r2 = subprocess.run(
                    ["fc-match", "--format=%{family}", name],
                    capture_output=True, text=True, timeout=5)
                family = r2.stdout.strip().lower()
                # Accept if any word of the requested name appears in family
                words = [w.lower() for w in name.split() if len(w) > 2]
                if words and any(w in family for w in words):
                    return path
                # Also accept if stem of the file path matches
                stem = Path(path).stem.lower().replace("-","").replace("_","")
                req  = name.lower().replace(" ","").replace("-","").replace("_","")
                if req in stem or stem in req:
                    return path
        ...
        for d in dirs:
            ...
            for ext_pat in ("*.ttf", "*.otf"):
                for f in d.rglob(ext_pat):
                    stem = f.stem.lower().replace("-","").replace("_","")
                    if safe == stem:         return str(f)   # exact match
                    if safe in stem and best is None:
                        best = str(f)                        # partial match
        return best
